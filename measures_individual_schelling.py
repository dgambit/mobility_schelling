...
def _radius_of_gyration_individual(traj):
    ...
    center_of_mass = np.round_(np.mean(lats_lngs, axis=0))
    rg = np.sqrt(np.mean([distance((lat, lng), center_of_mass) ** 2.0 for lat, lng in lats_lngs]))
    return rg
...
def _k_radius_of_gyration_individual(traj, k=2):
    ...
    top_k_locations = traj.drop_duplicates(subset=[constants.LATITUDE, constants.LONGITUDE]).sort_values(
        by=['visits', constants.DATETIME],
    ...
    krg = np.sqrt(sum([visits[i] * (distance((lat, lng), center_of_mass) ** 2.0)
...
def k_radius_of_gyration(traj, k=2, show_progress=True):
    ...
    # if 'uid' column in not present in the TrajDataFrame
    if constants.UID not in traj.columns:
        return pd.DataFrame([_k_radius_of_gyration_individual(traj, k=k)],
                            columns=['%s%s' % (k, sys._getframe().f_code.co_name)])
...
def _uncorrelated_entropy_individual(traj, normalize=False):
    ...
    probs = [1.0 * len(group) / n for group in
             traj.groupby(by=[constants.LATITUDE, constants.LONGITUDE]).groups.values()]
...
def uncorrelated_entropy(traj, normalize=False, show_progress=True):
    ...
    if show_progress:
        df = traj.groupby(constants.UID).progress_apply(
            lambda x: _uncorrelated_entropy_individual(x, normalize=normalize))
    else:
...
def _jump_lengths_individual(traj):
    ...
    lengths = np.array([distance(lats_lngs[i], lats_lngs[i - 1]) for i in range(1, len(lats_lngs))])
    return lengths
...
def _home_location_individual(traj, start_night='22:00', end_night='07:00'):
    ...
    if len(night_visits) != 0:
        lat, lng = \
        night_visits.groupby([constants.LATITUDE, constants.LONGITUDE]).count().sort_values(by=constants.DATETIME,
                                                                                            ascending=False).iloc[
            0].name
    else:
        lat, lng = traj.groupby([constants.LATITUDE, constants.LONGITUDE]).count().sort_values(by=constants.DATETIME,
                                                                                               ascending=False).iloc[
            0].name
    home_coords = (lat, lng)
    return home_coords
...
def home_location(traj, start_night='22:00', end_night='07:00', show_progress=True):
    ...
    # if 'uid' column in not present in the TrajDataFrame
    if constants.UID not in traj.columns:
        return pd.DataFrame([_home_location_individual(traj, start_night=start_night, end_night=end_night)],
                            columns=[constants.LATITUDE, constants.LONGITUDE])

    if show_progress:
        df = traj.groupby(constants.UID).progress_apply(
            lambda x: _home_location_individual(x, start_night=start_night, end_night=end_night))
    else:
        df = traj.groupby(constants.UID).apply(
            lambda x: _home_location_individual(x, start_night=start_night, end_night=end_night))
    return pd.DataFrame(df.to_list(), index=df.index).reset_index().rename(
        columns={0: constants.LATITUDE, 1: constants.LONGITUDE})
...
def _max_distance_from_home_individual(traj, start_night='22:00', end_night='07:00'):
    ...
    lengths = np.array(
        [distance((lat, lng), (home_lat, home_lng)) for i, (lat, lng) in enumerate(lats_lngs)])
    return lengths.max()
...
def max_distance_from_home(traj, start_night='22:00', end_night='07:00', show_progress=True):
    ...
    if constants.UID not in traj.columns:
        return pd.DataFrame([_max_distance_from_home_individual(traj,
                                                                ...
                                                                end_night=end_night)],
                            columns=[sys._getframe().f_code.co_name])

    if show_progress:
        df = traj.groupby(constants.UID).progress_apply(
            lambda x: _max_distance_from_home_individual(x, start_night=start_night, end_night=end_night))
    else:
        df = traj.groupby(constants.UID).apply(
            lambda x: _max_distance_from_home_individual(x, start_night=start_night, end_night=end_night))
    return pd.DataFrame(df).reset_index().rename(columns={0: sys._getframe().f_code.co_name})
...
def location_frequency(traj, normalize=True, as_ranks=False, show_progress=True,
    ...
    if show_progress:
        df = pd.DataFrame(traj.groupby(constants.UID)
                          .progress_apply(
            lambda x: _location_frequency_individual(x, normalize=normalize, location_columns=location_columns)))
    else:
        df = pd.DataFrame(traj.groupby(constants.UID)
                          .apply(
            lambda x: _location_frequency_individual(x, normalize=normalize, location_columns=location_columns)))
...
def individual_mobility_network(traj, self_loops=False, show_progress=True):
    ...
    if show_progress:
        return traj.groupby(constants.UID).progress_apply(lambda x: _individual_mobility_network_individual(x,
                                                                                                            self_loops=self_loops)).reset_index().drop(
            'level_1', axis=1)
    else:
...
def _frequency_rank_individual(traj):
    ...
    """
    traj = traj.groupby([constants.LATITUDE, constants.LONGITUDE]).count().sort_values(by=constants.DATETIME,
